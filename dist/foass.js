...
module.exports = function(robot) {
  return robot.hear(/^fu(?:\s)(?=([\w ]+))/i, function(msg) {
    var from, options, random_fu, to, url;
    options = ['off', 'you', 'donut', 'shakespeare', 'linus', 'king', 'chainsaw', 'outside', 'madison', 'nugget', 'yoda'];
    ...
    if (to) {
      ...
      options = ['this', 'that', 'everything', 'everyone', 'pink', 'life', 'thanks', 'flying', 'fascinating', 'cool', 'what', 'because'];
      random_fu = msg.random(options);
      // call /option/to
      url = `http://foaas.com/${random_fu}/${from}/`;
    }
    return msg.http(url).headers({
      ...
      try {
        ...
        return msg.send(`${json.message}\n${json.subtitle}`);
      } catch (error1) {
        error = error1;
        return msg.send("Fuck this error!");
      }
    });
  });
};
