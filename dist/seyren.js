...
module.exports = function(robot) {
  return robot.router.post("/hubot/seyren/alert", function(req, res) {
    var alert, alerts, check, i, j, len, len1, message, results, room, rooms, seyrenUrl;
    ...
    for (j = 0, len1 = rooms.length; j < len1; j++) {
      room = rooms[j];
      results.push(robot.messageRoom(room, message));
    }
    return results;
  });
};
