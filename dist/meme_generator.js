...
// Author:
//   skalnik
var inspect, khanify, memeGenerator, memeResponder;

inspect = require('util').inspect;

module.exports = function(robot) {
  var i, len, meme, ref;
  if (robot.brain.data.memes == null) {
  ...
  ref = robot.brain.data.memes;
  for (i = 0, len = ref.length; i < len; i++) {
    meme = ref[i];
    memeResponder(robot, meme);
  }
  robot.respond(/(memegen )?add meme \/(.+)\/i,(.+),(.+)/i, function(msg) {
    meme = {
    ...
    return memeResponder(robot, meme);
  });
  robot.respond(/(memegen )?k(?:ha|ah)nify (.*)/i, function(msg) {
    if (Math.random() > 0.5) {
      // Kirk khan
      return memeGenerator(msg, 6443, 1123022, "", khanify(msg.match[2]), function(url) {
        return msg.send(url);
      });
    } else {
      // Spock khan
      return memeGenerator(msg, 2103732, 8814557, "", khanify(msg.match[2]), function(url) {
        return msg.send(url);
      });
    }
  });
  robot.respond(/(memegen )?(IF .*), ((ARE|CAN|DO|DOES|HOW|IS|MAY|MIGHT|SHOULD|THEN|WHAT|WHEN|WHERE|WHICH|WHO|WHY|WILL|WON\'T|WOULD)[ \'N].*)/i, function(msg) {
    return memeGenerator(msg, 17, 984, msg.match[2], msg.match[3] + (msg.match[3].search(/\?$/) === (-1) ? '?' : ''), function(url) {
      return msg.send(url);
    });
  });
  return robot.respond(/(memegen )?((Oh|You) .*) ((Please|Tell) .*)/i, function(msg) {
    return memeGenerator(msg, 542616, 2729805, msg.match[2], msg.match[4], function(url) {
      return msg.send(url);
    });
  });
};

memeResponder = function(robot, meme) {
  return robot.respond(meme.regex, function(msg) {
    return memeGenerator(msg, meme.generatorID, meme.imageID, msg.match[2], msg.match[3], function(url) {
      return msg.send(url);
    });
  });
};
...
memeGenerator = function(msg, generatorID, imageID, text0, text1, callback) {
  var password, preferredDimensions, username;
  ...
  return msg.http('http://version1.api.memegenerator.net/Instance_Create').query({
    ...
    var errorMessage, img, instanceID, instanceURL, jsonBody, result, success;
    if (err) {
    ...
    instanceID = result != null ? result.instanceID : void 0;
    instanceURL = result != null ? result.instanceUrl : void 0;
    img = result != null ? result.instanceImageUrl : void 0;
    if (!(instanceID && instanceURL && img)) {
    ...
    return msg.http(instanceURL).get()(function(err, res, body) {
      ...
      if (preferredDimensions != null) {
        return callback(`http://images.memegenerator.net/instances/${preferredDimensions}/${instanceID}.jpg`);
      } else {
        return callback(`http://images.memegenerator.net/instances/${instanceID}.jpg`);
      }
    });
  });
};

khanify = function(msg) {
  var i, index, len, v, vowels;
  msg = msg.toUpperCase();
  vowels = ['A', 'E', 'I', 'O', 'U'];
  index = -1;
  for (i = 0, len = vowels.length; i < len; i++) {
    v = vowels[i];
    if (msg.lastIndexOf(v) > index) {
      index = msg.lastIndexOf(v);
    }
  }
  return `${msg.slice(0, index)}${Array(10).join(msg.charAt(index))}${msg.slice(index)}!!!!!`;
};
