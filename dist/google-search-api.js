...
module.exports = function(robot) {
  return robot.respond(/(google|search)( me)? (.*)/i, function(msg) {
    return msg.http("https://www.googleapis.com/customsearch/v1").query({
      ...
      if (results.error) {
        results.error.errors.forEach(function(err) {
          return resp += err.message;
        });
      } else {
        results.items.forEach(function(item) {
          return resp += item.title + " - " + item.link + "\n";
        });
      }
