...
secrets = [`My token is ${process.env.HUBOT_CAMPFIRE_TOKEN}.`, `I'm currently in room(s) ${process.env.HUBOT_CAMPFIRE_ROOMS}, with a room name of ${process.env.HUBOT_CAMPFIRE_ROOM_NAME}.`, `The subdomain is ${process.env.HUBOT_CAMPFIRE_ACCOUNT}.`];

module.exports = function(robot) {
  return robot.respond(/secrets/i, function(msg) {
    return msg.send(secrets.join('\n'));
  });
};
