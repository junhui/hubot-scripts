...
// Author:
//   dzello
var fromFullCharCode, substituteEmoji, toEmoji;

module.exports = function(robot) {
  robot.brain.on("loaded", function() {
    var base;
    return (base = robot.brain.data).emojis != null ? base.emojis : base.emojis = {};
  });
  robot.hear(/.+/, function(msg) {
    var textWithEmoji;
    if (textWithEmoji = substituteEmoji(msg.message.text, robot)) {
      return msg.send(textWithEmoji);
    }
  });
  robot.respond(/more emoji/, function(msg) {
    return robot.brain.data.emojis.more = true;
  });
  robot.respond(/less emoji/, function(msg) {
    return robot.brain.data.emojis.more = false;
  });
  return robot.respond(/list emoji/, function(msg) {
    var buf, hexCode, symbol;
    buf = "";
    for (symbol in emojis) {
      hexCode = emojis[symbol];
...
substituteEmoji = function(text, robot) {
  var ct, re;
  ct = 0;
  re = robot.brain.data.emojis.more ? /([a-zA-Z0-9_:]{3,})/g : /:([a-zA-Z0-9_]+):/g;
  text = text.replace(re, function(match, key) {
    var emoji;
...
toEmoji = function(hexCode) {
  return fromFullCharCode(parseInt(hexCode, 16));
};

fromFullCharCode = function(...args) {
  var chars, high, i, len, low, n;
  chars = [];
