...
module.exports = function(robot) {
  robot.respond(/it's been (\d+) days since\s+(.*?)[.?!]?$/i, function(msg) {
    var base, date;
    date = new Date;
    date.setTime(date.getTime() - (parseInt(msg.match[1]) * 1000 * 24 * 60 * 60));
    date.setHours(0, 0, 0, 0);
    (base = robot.brain.data).days_since || (base.days_since = {});
    robot.brain.data.days_since[msg.match[2]] = date;
    return msg.send("okay, it's been " + msg.match[1] + " days since " + msg.match[2]);
  });
  return robot.respond(/how long since\s+(.*?)\??$/i, function(msg) {
    var date, days_since;
    if (robot.brain.data.days_since && robot.brain.data.days_since[msg.match[1]]) {
      ...
      return msg.send("it's been " + days_since + " days since " + msg.match[1]);
    } else {
      return msg.send("I don't recall that event");
    }
  });
};
