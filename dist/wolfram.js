// Description:
//   Allows hubot to answer almost any question by asking Wolfram Alpha

// Dependencies:
//   "wolfram": "0.2.2"

// Configuration:
//   HUBOT_WOLFRAM_APPID - your AppID

// Commands:
//   hubot question <question> - Searches Wolfram Alpha for the answer to the question

// Notes:
//   This may not work with node 0.6.x

// Author:
//   dhorrigan
var Wolfram;

Wolfram = require('wolfram').createClient(process.env.HUBOT_WOLFRAM_APPID);

module.exports = function(robot) {
  return robot.respond(/(question|wfa) (.*)$/i, function(msg) {
    console.log(msg.match);
    return Wolfram.query(msg.match[2], function(e, result) {
      // console.log result
      if (result && result.length > 0) {
        return msg.send(result[1]['subpods'][0]['value']);
      } else {
        return msg.send('Hmm...not sure');
      }
    });
  });
};
