...
module.exports = function(robot) {
  robot.respond(/grumpycat me/i, function(msg) {
    return msg.send(cats[Math.floor(Math.random() * cats.length)]);
  });
  robot.respond(/grumpycat bomb( (\d+))?/i, function(msg) {
    var count, i, j, ref, results;
    count = msg.match[2] || 5;
    results = [];
    for (i = j = 1, ref = count; j <= ref; i = j += 1) {
      results.push(msg.send(cats[Math.floor(Math.random() * cats.length)]));
    }
    return results;
  });
  return robot.respond(/how many grumpycats are there/i, function(msg) {
    return msg.send(`There are ${cats.length} grumpycats.`);
  });
};
