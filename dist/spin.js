...
// Author:
//   mmstud
var access_key, capture, crypto, env, get_signature, gmdatenow, hear_and_respond, host, match_regexp_str, path, replace_regexp_str, scheme, secret_key, spin, split_str, spun, version;
...
get_signature = function(host, path, secret_key, access_key, timestamp, version) {
  var data, hmac, newline, request;
  newline = "\n";
...
spun = function(msg, spinner = null, robot_brain = null) {
  var signature, timestamp;
  timestamp = gmdatenow();
  signature = get_signature(host, path, secret_key, access_key, timestamp, version);
  return msg.http(scheme + "://" + host + path).query({
    timestamp: timestamp,
    version: version,
    access_key: access_key,
    signature: signature,
    type: 'json'
  }).post(msg.match[1])(function(err, res, body) {
    if (err) {
      return msg.send(err);
    } else if (res.statusCode === !200) {
      return msg.send("Service not available! Status code: " + res.statusCode);
    } else {
      body = JSON.parse(body);
      if (body.response === 'error') {
        return msg.send(body.message);
      } else {
        if (spinner) {
          return msg.send(spinner(body.message));
        } else {
          // set to the memory so user can spin it with separate "spin the last spun" command
          if (robot_brain) {
            if (!robot_brain.data.spin) {
...
module.exports = function(robot) {
  hear_and_respond(robot, 'spin me ([\\s\\S]*)', function(msg) {
    return msg.send(spin(msg.match[1]));
  });
  hear_and_respond(robot, 'spun me ([\\s\\S]*)', function(msg) {
    return spun(msg, null, robot.brain);
  });
  hear_and_respond(robot, 'spun and spin me ([\\s\\S]*)', function(msg) {
    return spun(msg, spin);
  });
  return hear_and_respond(robot, 'spin the last spun$', function(msg) {
    // get user centric input from memory
    if (this.robot.brain.data.spin[msg.message.user.reply_to]) {
      return msg.send(spin(robot.brain.data.spin[msg.message.user.reply_to]));
    } else {
      return msg.send("Nothing so spin. Please use spun first.");
    }
  });
};
